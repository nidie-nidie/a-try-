...
/* 更新旋转矩阵 */ // 计算从地理导航坐标系(n系)到机体坐标系(b系)的旋转矩阵 (方向余弦矩阵DCM)
...
void mahony_update(struct MAHONY_FILTER_t *mahony_filter)
{
    ...
    /* 陀螺仪角速度转换 */
    //    mahony_filter->gyro.x *= DEG2RAD;        /* 转换为弧度 */
    ...
    /* 提取旋转矩阵中的重力方向，与加速度计测量的重力方向求叉积，计算出误差 */
...
{
    /* 计算 roll(横滚角) pitch(俯仰角) yaw(偏航角) 欧拉角 */
